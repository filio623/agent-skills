---
name: mem0
description: Use this skill when adding or maintaining Mem0 persistent memory in AI applications, including MemoryClient, memory layers, user preference memory, personalization, semantic memory search, Python or TypeScript mem0ai SDK usage, and framework integrations such as LangChain, CrewAI, OpenAI Agents SDK, Pipecat, LlamaIndex, AutoGen, and LangGraph.
license: Apache-2.0
metadata:
  author: mem0ai
  version: "3.0.0"
...
Use this as the default Mem0 skill for ambiguous Mem0 requests. If a project has separate local skills for the Mem0 CLI or Vercel AI SDK provider, prefer those for command-line workflows or `@mem0/vercel-ai-provider` work.
...
```bash
python skills/mem0/scripts/mem0_doc_search.py --query "topic"
...
Adjust the path if this skill is installed somewhere other than `skills/mem0`. No API key is needed; the script searches docs.mem0.ai directly.
